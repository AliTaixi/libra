...
export function useThreadStream({
  ...
  const sendMessage = useCallback(
    async (
      ...
      // Also show existing files (referenced, not uploaded) as already uploaded
      if (message.existingFiles) {
        for (const ref of message.existingFiles) {
          optimisticFiles.push({
            filename: ref.filename,
            size: ref.size,
            path: `/mnt/user-data/${ref.filename}`,
      ...
      try {
        ...
        // Add files that already exist in user-data (no upload needed)
        if (message.existingFiles && message.existingFiles.length > 0) {
          for (const ref of message.existingFiles) {
            filesForSubmit.push({
              filename: ref.filename,
              size: ref.size,
        ...
        // Also inject file paths into message text as <user_files> tags,
        // so the AI can see them even if additional_kwargs.files is lost.
        let finalText = text;
        if (filesForSubmit.length > 0) {
          const refBlock = filesForSubmit
            .map((f) => `/mnt/user-data/${f.filename}`)
            .join("\n");
          finalText = `${text}\n\n<user_files>\n${refBlock}\n</user_files>`;
        }
