...
def normalize_filename(filename: str) -> str:
    ...
    Strips directory components, rejects traversal patterns, and replaces
    characters that cause AI output mismatches (spaces, parentheses) with
    underscores so that filenames are easy for AI agents to reference
    correctly in generated content (e.g. Markdown image paths).

    Normalization rules:
    - Strips any directory components (basename only).
    - Rejects traversal patterns, backslashes, overly long names.
    - ASCII parentheses ``()`` → ``_``
    - Full-width parentheses ``（）`` → ``_``
    - Spaces → ``_``
    ...
    Returns:
        Safe, normalized filename (basename only).
    ...
    # Normalize special characters that trip up AI filename references
    # ASCII and full-width parentheses → underscore
...
def upload_artifact_url(thread_id: str, filename: str) -> str:
    ...
    *filename* is percent-encoded so that spaces, ``#``, ``?`` etc. are safe.
    Files now sit flat under user-data (no subdirectory).
    """
    return f"/api/threads/{thread_id}/artifacts{VIRTUAL_PATH_PREFIX}/{quote(filename, safe='')}"


def upload_virtual_path(filename: str) -> str:
    """Build the virtual path for a file in the flat user-data directory."""
    return f"{VIRTUAL_PATH_PREFIX}/{filename}"
