...
async def _skill_manage_impl(
    ...
    async with lock:
        if action == "create":
            ...
            # Auto-add YAML frontmatter if missing (AI agents often forget it)
            if not content.startswith("---"):
                first_line = content.strip().split("\n")[0] if content.strip() else name
                desc = first_line[:80] if len(first_line) > 5 else f"Custom skill: {name}"
                frontmatter = f"---\nname: {name}\ndescription: {desc}\n---\n\n"
...
async def skill_manage_tool(
    ...
    Args:
        action: One of create, patch, edit, delete, write_file, remove_file.
        name: Skill name in hyphen-case (e.g. "my-translator").
        content: SKILL.md content (YAML frontmatter with name/description + markdown body). If frontmatter is missing, it will be auto-generated using the skill name.
        path: Supporting file path for write_file or remove_file.
        find: Existing text to replace for patch.
        replace: Replacement text for patch.
        expected_count: Optional expected number of replacements for patch.
