...
def _restore_reasoning_content(payload_msg: dict, orig_msg: AIMessage) -> None:
    """Re-inject ``reasoning_content`` onto assistant messages.

    DeepSeek's thinking mode requires ``reasoning_content`` to be present on
    ALL assistant messages in multi-turn conversations. LangChain stores it
    in ``additional_kwargs["reasoning_content"]`` but drops it during
    serialisation. This function restores it before sending to the API.
    """
