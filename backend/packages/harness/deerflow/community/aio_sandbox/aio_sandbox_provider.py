...
class AioSandboxProvider(SandboxProvider):
    ...
    def _get_thread_mounts(thread_id: str) -> list[tuple[str, str, bool]]:
        ...
        User-data is now per-user and shared across threads: one flat directory
        mounted at `/mnt/user-data/` in the sandbox.
        ...
        # Single mount for per-user shared user-data (flat, all files together)
        user_data_host = paths.host_user_data_dir(user_id) if user_id else paths.host_sandbox_user_data_dir(thread_id)

        return [
            (user_data_host, VIRTUAL_PATH_PREFIX, False),
