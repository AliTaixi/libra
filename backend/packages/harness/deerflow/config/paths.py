...
class Paths:
    ...
    Directory layout (host side):
        ...
        └── users/                         <-- per-user data
            └── {user_id}/
                ...
                │   (flat directory — all files from uploads, workspace, and outputs
                │    are shared across all threads for this user)
                └── threads/
                    └── {thread_id}/
                        └── acp-workspace/ <-- per-thread ACP workspace
    ...
    def user_data_dir(self, user_id: str) -> Path:
        """
        Per-user shared user-data directory.

        All user-facing files (uploads, workspace, outputs) live in this single
        flat directory, shared across all threads for this user.

        Host: `{base_dir}/users/{user_id}/user-data/`
        Sandbox: `/mnt/user-data/`

        Raises:
            ValueError: If *user_id* contains unsafe characters.
        """
        return self.user_dir(user_id) / "user-data"
    ...
    def thread_dir(self, thread_id: str, *, user_id: str | None = None) -> Path:
        ...
        This directory now contains only thread-specific data (e.g. ACP workspace).
        The shared user-data/ directory is at the user level.
    ...
    def sandbox_work_dir(self, thread_id: str, *, user_id: str | None = None) -> Path:
        ...
        All user files are now per-user, shared across threads.
        User-data is at `{base_dir}/users/{user_id}/user-data/`.
        Sandbox: `/mnt/user-data/`
        """
        if user_id is not None:
            return self.user_data_dir(user_id)
        return self.thread_dir(thread_id) / "user-data"  # legacy fallback
    ...
    def acp_workspace_dir(self, thread_id: str, *, user_id: str | None = None) -> Path:
        ...
        Host: `{base_dir}/users/{user_id}/threads/{thread_id}/acp-workspace/`
    ...
    def sandbox_user_data_dir(self, thread_id: str, *, user_id: str | None = None) -> Path:
        ...
        Now per-user shared: `{base_dir}/users/{user_id}/user-data/`.
        Sandbox: `/mnt/user-data/`
        """
        if user_id is not None:
            return self.user_data_dir(user_id)
    ...
    def host_user_data_dir(self, user_id: str) -> str:
        """Host path for the per-user shared user-data directory."""
        return _join_host_path(self._host_base_dir_str(), "users", _validate_user_id(user_id), "user-data")

    def host_sandbox_user_data_dir(self, thread_id: str, *, user_id: str | None = None) -> str:
        """Host path for the user-data root — now per-user shared."""
        if user_id is not None:
            return self.host_user_data_dir(user_id)
        return _join_host_path(self.host_thread_dir(thread_id), "user-data")  # legacy fallback

    def host_sandbox_work_dir(self, thread_id: str, *, user_id: str | None = None) -> str:
        """Host path for the workspace mount source — same as user-data."""
        return self.host_sandbox_user_data_dir(thread_id, user_id=user_id)

    def host_sandbox_uploads_dir(self, thread_id: str, *, user_id: str | None = None) -> str:
        """Host path for the uploads mount source — same as user-data."""
        return self.host_sandbox_user_data_dir(thread_id, user_id=user_id)

    def host_sandbox_outputs_dir(self, thread_id: str, *, user_id: str | None = None) -> str:
        """Host path for the outputs mount source — same as user-data."""
        return self.host_sandbox_user_data_dir(thread_id, user_id=user_id)
    ...
    def ensure_thread_dirs(self, thread_id: str, *, user_id: str | None = None) -> None:
        ...
        The shared ``user-data/`` directory lives at the user level, so it is
        created once (first call).  The ACP workspace remains per-thread.
        ...
        """
        # Per-user shared user-data dir (created lazily, idempotent)
        if user_id is not None:
            ud_dir = self.user_data_dir(user_id)
            ud_dir.mkdir(parents=True, exist_ok=True)
            ud_dir.chmod(0o777)
        else:
            # Legacy fallback: per-thread user-data
            legacy_dir = self.sandbox_user_data_dir(thread_id)
            legacy_dir.mkdir(parents=True, exist_ok=True)
            legacy_dir.chmod(0o777)

        # ACP workspace is still per-thread
        acp_dir = self.acp_workspace_dir(thread_id, user_id=user_id)
    ...
    def delete_thread_dir(self, thread_id: str, *, user_id: str | None = None) -> None:
        ...
        Note: user-data is shared across threads and is NOT deleted here.
        """
    ...
    def resolve_virtual_path(self, thread_id: str, virtual_path: str, *, user_id: str | None = None) -> Path:
        ...
        Args:
            thread_id: The thread ID (ignored for user-scoped resolution when user_id is provided).
            virtual_path: Virtual path as seen inside the sandbox, e.g.
                          ``/mnt/user-data/report.pdf``.
