...
def _create_summarization_middleware(*, app_config: AppConfig | None = None) -> DeerFlowSummarizationMiddleware | None:
    ...
    if config.model_name:
        model = create_chat_model(name=config.model_name, app_config=resolved_app_config)
    else:
        model = create_chat_model(app_config=resolved_app_config)
...
def _make_lead_agent(config: RunnableConfig, *, app_config: AppConfig):
    ...
    logger.info(
        "Create Agent(%s) -> model_name: %s, is_plan_mode: %s, subagent_enabled: %s, max_concurrent_subagents: %s",
