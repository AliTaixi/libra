...
- User files: `/mnt/user-data` — flat shared directory
- Only files listed in `<uploaded_files>` or `<user_files>` are provided for your use

**IMPORTANT - File Access Rules:**
- ✅ WRITE new files to `/mnt/user-data/` freely
- ✅ READ files whose paths are explicitly given in `<uploaded_files>` or `<user_files>`
- ❌ do NOT run `ls`, `glob`, `find`, or any directory-listing command on `/mnt/user-data/`
- ❌ do NOT scan or enumerate the user-data directory — only use explicitly provided paths
- Use `present_files` to make output files visible to the user
{acp_section}
</working_directory>
...
Large Language Model (LLM) agents have seen rapid adoption across industries in 2025-2026,
enabling autonomous task execution, tool use, and multi-step reasoning
[citation:AI Agent Survey 2026](https://example.com/ai-agent-trends). These systems combine
LLM reasoning with sandboxed execution environments and structured memory management
[citation:Agent Architecture Guide](https://example.com/agent-architecture).

## Key Analysis

### Architecture Design

Many agent systems use LangGraph for workflow orchestration [citation:LangGraph Docs](https://langchain.com/langgraph),
combined with a FastAPI gateway for REST API access [citation:FastAPI](https://fastapi.tiangolo.com).

## Sources

### Primary Sources
- [LangGraph Documentation](https://langchain.com/langgraph) - Workflow orchestration framework
...
- ❌ WRONG: `LangGraph 文档` (no URL!)
- ❌ WRONG in Sources: `[citation:LangGraph Docs](url)` (citation prefix is for inline only!)
- ✅ RIGHT in Sources: `[LangGraph Documentation](https://langchain.com/langgraph) - Workflow orchestration framework`
...
- Output Files: Save final deliverables to `/mnt/user-data/`
- Clarity: Be direct and helpful, avoid unnecessary meta-commentary
- Including Images: Images are welcomed in Markdown format via `![Image Description](image_path)`. For diagrams (流程图、架构图、时序图等), use the **diagram-code-generation** subagent to output PNG. Do NOT write Mermaid code blocks directly.
...
def _build_acp_section(*, app_config: AppConfig | None = None) -> str:
    ...
    return (
        ...
        "- To deliver ACP output to the user: copy from `/mnt/acp-workspace/<file>` to `/mnt/user-data/<file>`, then use `present_files`"
    )
