...
class ThreadDataMiddleware(AgentMiddleware[ThreadDataMiddlewareState]):
    """Provide thread data directory paths for agent execution.

    All user files (uploads, workspace, outputs) now share a single flat
    per-user directory: `{base_dir}/users/{user_id}/user-data/`.
    ...
    def _get_thread_paths(self, thread_id: str, user_id: str | None = None) -> dict[str, str]:
        """Get the paths for user data (all pointing to the same flat shared dir).
        ...
        Returns:
            ...
            (all three point to the same per-user user-data directory).
        """
        return {
    ...
    def before_agent(self, state: ThreadDataMiddlewareState, runtime: Runtime) -> dict | None:
        ...
        # Initialize allowed_read_paths to empty list if not set,
        # so that no files are readable until explicitly referenced
        # (via uploads or present_files).
        state_updates: dict = {
            "thread_data": {
                "thread_id": thread_id,
                **paths,
        ...
        if state.get("allowed_read_paths") is None:
            state_updates["allowed_read_paths"] = []
