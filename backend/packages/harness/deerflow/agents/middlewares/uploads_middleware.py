...
def _get_message_text(message: HumanMessage) -> str:
    """Extract plain text content from a HumanMessage (string or list format)."""
...
class UploadsMiddleware(AgentMiddleware[UploadsMiddlewareState]):
    ...
    def _format_file_entry(self, file: dict, lines: list[str]) -> None:
        ...
        if outline:
            ...
            if preview:
                ...
                for text in preview:
                    lines.append(f"    > {text}")
            lines.append("  Use `grep` to search for keywords (e.g. `grep(pattern='keyword', path='/mnt/user-data/')`).")
        lines.append("")
    ...
    def _create_files_message(self, new_files: list[dict], historical_files: list[dict]) -> str:
        ...
        Args:
            ...
            historical_files: Kept for signature compatibility but always empty
                              (middleware no longer auto-scans user-data).
        ...
        lines.append("The following files are attached to this message:")
        lines.append("")
        if new_files:
        ...
        lines.append("  (e.g. `grep(pattern='revenue', path='/mnt/user-data/')`).")
        lines.append("- Use `glob` to find files by name pattern")
        lines.append("  (e.g. `glob(pattern='**/*.md', path='/mnt/user-data/')`).")
    ...
    def before_agent(self, state: UploadsMiddlewareState, runtime: Runtime) -> dict | None:
        ...
        ONLY files explicitly referenced in the current message's
        additional_kwargs.files are included — the middleware no longer
        scans the shared user-data directory. This ensures the agent
        only sees files the user chose to attach.
        ...
        # Check for files from additional_kwargs (standard upload flow)
        new_files = self._files_from_kwargs(last_message, uploads_dir) or []

        # Also check for <user_files> tags in the message text (reference flow)
        text_files: list[dict] = []
        content_text = _get_message_text(last_message)
        if content_text and "<user_files>" in content_text:
            import re as _re
            for m in _re.finditer(r"<user_files>\n(.*?)\n</user_files>", content_text, _re.DOTALL):
                for line in m.group(1).strip().split("\n"):
                    line = line.strip()
                    if not line:
                        continue
                    fname = Path(line).name
                    if uploads_dir and not (uploads_dir / fname).is_file():
                        continue
                    # Avoid duplicates
                    if any(f["filename"] == fname for f in new_files):
                        continue
                    file_size = 0
                    if uploads_dir:
                        try:
                            file_size = (uploads_dir / fname).stat().st_size
                        except OSError:
                            pass
                    text_files.append({
                        "filename": fname,
                        "size": file_size,
                        "path": f"/mnt/user-data/{fname}",
                        "extension": Path(fname).suffix,
                    })

        all_files = new_files + text_files

        # Attach outlines to all files
        if uploads_dir:
        ...
        logger.debug(f"Uploaded files: {[f['filename'] for f in all_files]}")

        # Create files message and prepend to the last human message content
        files_message = self._create_files_message(all_files, [])
        ...
        # Seed allowed_read_paths so the agent can read uploaded files
        allowed_paths = [f"/mnt/user-data/{f['filename']}" for f in all_files]
