    ...
    async def put(self, *, thread_id, run_id, event_type, category, content="", metadata=None, created_at=None):  # noqa: D401
        ...
        async with self._sf() as session:
            async with session.begin():
                # Lock existing rows to serialize seq assignment within a thread.
                # NOTE: SELECT MAX(...) FOR UPDATE is rejected by PostgreSQL,
                # so we select the seq values directly and compute max in Python.
                existing = await session.scalars(
                    select(RunEventRow.seq).where(RunEventRow.thread_id == thread_id).with_for_update()
                )
                seq_values = existing.all()
                max_seq = max(seq_values) if seq_values else 0
                seq = max_seq + 1
                row = RunEventRow(
                    thread_id=thread_id,
                    run_id=run_id,
    ...
    async def put_batch(self, events):
        ...
        async with self._sf() as session:
            async with session.begin():
                ...
                seq_values = existing.all()
                seq = max(seq_values) if seq_values else 0
                rows = []
                for e in events:
                    seq += 1
