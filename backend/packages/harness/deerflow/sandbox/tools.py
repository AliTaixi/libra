...
from deerflow.uploads.manager import normalize_filename as _normalize_filename
...
def _extract_thread_id_from_thread_data(thread_data: "ThreadDataState | None") -> str | None:
    ...
    Prefers the explicit ``thread_id`` field (set by ThreadDataMiddleware).
    Falls back to extracting from workspace_path for legacy thread states.
    """
    if thread_data is None:
        return None
    # Prefer the explicit thread_id field (set by ThreadDataMiddleware)
    tid = thread_data.get("thread_id")
    if tid:
        return tid
    # Fallback: legacy layout — path-based extraction
    # Old: {base_dir}/.../threads/{thread_id}/user-data/workspace
...
def _resolve_local_read_path(path: str, thread_data: ThreadDataState, allowed_read_paths: list[str] | None = None) -> str:
    validate_local_tool_path(path, thread_data, read_only=True, allowed_read_paths=allowed_read_paths)
...
def replace_virtual_path(path: str, thread_data: ThreadDataState | None) -> str:
    ...
    Mapping:
        /mnt/user-data/* -> thread_data['workspace_path']/*  (shared per-user data — all files flat)
...
def _thread_virtual_to_actual_mappings(thread_data: ThreadDataState) -> dict[str, str]:
    ...
    All user files (uploads, workspace, outputs) share a single flat
    per-user directory.  The virtual root ``/mnt/user-data`` maps directly
    to that directory — no subdirectory mappings are needed.
    """
    mappings: dict[str, str] = {}

    # Any of workspace/uploads/outputs works — they all point to the same
    # flat per-user ``user-data/`` directory.
    base_path = thread_data.get("workspace_path") or thread_data.get("uploads_path") or thread_data.get("outputs_path")
    if base_path:
        mappings[VIRTUAL_PATH_PREFIX] = base_path
...
def validate_local_tool_path(
    ...
    *,
    ...
    Allowed virtual-path families:
      - ``/mnt/user-data/*``  — allowed for write; read access restricted to
        ``allowed_read_paths`` when the list is non-empty
    ...
    Args:
        ...
        read_only: When True, skills and ACP workspace paths are permitted,
                   and user-data reads are restricted to ``allowed_read_paths``.
        allowed_read_paths: Virtual paths permitted for read access.  ``None``
                            means no restriction (backward compatible); an empty
                            list means no files may be read at all.
    ...
    # User-data paths — check read access restriction
    if path.startswith(f"{VIRTUAL_PATH_PREFIX}/"):
        if read_only and allowed_read_paths is not None:
            # allowed_read_paths = None  → no restriction (backward compat)
            # allowed_read_paths = []     → block all reads (no files referenced)
            # allowed_read_paths = [...]   → only listed paths may be read
            if not allowed_read_paths or path not in allowed_read_paths:
                raise PermissionError(
                    f"Access denied: file not referenced in conversation: {path}. "
                    f"Only files listed in <uploaded_files> or <user_files> may be read."
                )
        return
...
def _validate_resolved_user_data_path(resolved: Path, thread_data: ThreadDataState) -> None:
    """Verify that a resolved host path stays inside the user-data directory.

    Raises PermissionError if the path escapes the allowed root.
    """
    base = thread_data.get("workspace_path") or thread_data.get("uploads_path") or thread_data.get("outputs_path")
    if not base:
...
def get_allowed_read_paths(runtime: Runtime | None) -> list[str] | None:
    """Extract allowed_read_paths from runtime state."""
    if runtime is None or runtime.state is None:
        return None
    return runtime.state.get("allowed_read_paths")
...
# ── Banned operations (enforced at tool level, not just prompt) ──────────
_BANNED_PIP_RE = re.compile(
    r"(?:^|\s|;|&&)(?:pip|pip3|python\s+-m\s+pip)\s+install\b",
    re.IGNORECASE,
)
_BANNED_PYTHON_RE = re.compile(
    r"(?:^|\s|;|&&)(?:python3|python)\s+",
    re.IGNORECASE,
)
_BANNED_PY_FILES = re.compile(r"\.py$", re.IGNORECASE)


def _check_banned_command(command: str) -> str | None:
    """Return an error message if *command* uses a banned operation, else None."""
    if _BANNED_PIP_RE.search(command):
        return (
            "错误：禁止手动安装 Python 依赖。"
            "如果脚本缺少依赖，由脚本自身负责安装（如 convert_md_to_docx.sh 会自动安装 python-docx）。"
        )
    # Allow python to run pre-installed skill scripts under /mnt/skills/
    if _BANNED_PYTHON_RE.search(command):
        if "/mnt/skills/" in command:
            return None
        return (
            "错误：禁止直接运行 Python 代码。"
            "所有 Python 操作必须通过技能提供的脚本完成（如 convert_md_to_docx.sh）。"
        )
    return None


def _check_banned_file_path(path: str) -> str | None:
    """Return an error message if *path* is a banned file type, else None."""
    if _BANNED_PY_FILES.search(path):
        return "错误：禁止创建 .py 文件。请使用技能提供的脚本（如 convert_md_to_docx.sh）完成工作。"
    return None
...
def bash_tool(runtime: Runtime, description: str, command: str) -> str:
    ...
    - If a script reports a missing Python package, run the script again —
      it will auto-install its dependencies (e.g. convert_md_to_docx.sh).
    - Do NOT run pip install manually — it is blocked.

    Args:
        description: Explain why you are running this command in short words. ALWAYS PROVIDE THIS PARAMETER FIRST.
        command: The bash command to execute. Always use absolute paths for files and directories.
    """
    # Enforce banned-command check at the code level
    banned = _check_banned_command(command)
    if banned:
...
def ls_tool(runtime: Runtime, description: str, path: str) -> str:
    ...
    try:
        ...
        if is_local_sandbox(runtime):
            thread_data = get_thread_data(runtime)
            allowed_read_paths = get_allowed_read_paths(runtime)
            validate_local_tool_path(path, thread_data, read_only=True, allowed_read_paths=allowed_read_paths)
            if _is_skills_path(path):
    ...
    try:
        ...
        if is_local_sandbox(runtime):
            ...
            if thread_data is None:
                raise SandboxRuntimeError("Thread data not available for local sandbox")
            allowed_read_paths = get_allowed_read_paths(runtime)
            path = _resolve_local_read_path(path, thread_data, allowed_read_paths=allowed_read_paths)
...
def write_file_tool(
    ...
    """
    # Enforce banned-file check at the code level
    banned = _check_banned_file_path(path)
    if banned:
        return f"Error: {banned}"
    try:
        sandbox = ensure_sandbox_initialized(runtime)
        ...
        if is_local_sandbox(runtime):
            thread_data = get_thread_data(runtime)
            # Normalize filename for user-data paths so the AI sees the real path
            if sandbox_path.startswith(f"{VIRTUAL_PATH_PREFIX}/"):
                parent = sandbox_path.rsplit("/", 1)[0]
                filename = sandbox_path.rsplit("/", 1)[1]
                norm_name = _normalize_filename(filename)
                sandbox_path = f"{parent}/{norm_name}" if norm_name != filename else sandbox_path
            validate_local_tool_path(sandbox_path, thread_data)
            if not _is_custom_mount_path(sandbox_path):
                sandbox_path = _resolve_and_validate_user_data_path(sandbox_path, thread_data)
            # Custom mount paths are resolved by LocalSandbox._resolve_path()
        with get_file_operation_lock(sandbox, sandbox_path):
            sandbox.write_file(sandbox_path, content, append)
        if sandbox_path != path:
            return f"OK - wrote to {sandbox_path}"
        return "OK"
    except SandboxError as e:
        return f"Error: {e}"
    except PermissionError:
