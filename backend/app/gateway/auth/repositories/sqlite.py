...
class SQLiteUserRepository(UserRepository):
    ...
    async def has_privileged_users(self) -> bool:
        stmt = select(func.count()).select_from(UserRow).where(
            UserRow.system_role.in_(["admin", "super"])
        )
        async with self._sf() as session:
    ...
    async def count_super_admin_users(self) -> int:
        stmt = select(func.count()).select_from(UserRow).where(UserRow.system_role == "super")
        async with self._sf() as session:
            return await session.scalar(stmt) or 0
