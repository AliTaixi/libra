...
from app.gateway.deps import get_current_user_from_request, get_local_provider, require_min_role
...
async def setup_status(request: Request):
    ...
    has_privileged = await get_local_provider().has_privileged_users()
    return {"needs_setup": not has_privileged}
...
class UserManageResponse(BaseModel):
    """Response model for user management list."""

    id: str
    email: str
    system_role: Literal["admin", "user", "super"]
    created_at: str


@router.get("/users", response_model=list[UserManageResponse])
async def list_users(request: Request):
    """List all users. Requires admin or super role."""
    await require_min_role(request, "admin")
    provider = get_local_provider()
...
async def delete_user(user_id: str, request: Request):
    """Delete a regular user account. Requires admin or super role.

    - Admin can only delete users with 'user' role.
    - Super can delete any user except themselves.
    """
    caller = await require_min_role(request, "admin")
    ...
    if str(caller.id) == user_id:
        raise HTTPException(status_code=400, detail="不能删除自己的账号")

    # Admin can only delete regular users
    if caller.system_role == "admin" and target.system_role != "user":
        raise HTTPException(status_code=403, detail="管理员只能删除普通用户账号")
