...
class AgentResponse(BaseModel):
    ...
    category: str = Field(default="custom", description="public or custom")
...
def _agent_config_to_response(agent_cfg: AgentConfig, include_soul: bool = False, *, user_id: str | None = None) -> AgentResponse:
    ...
    # Determine if agent is public (shared) or custom (per-user)
    paths = get_paths()
    effective_user = user_id or get_effective_user_id()
    is_public = not paths.user_agent_dir(effective_user, agent_cfg.name).exists() and paths.agent_dir(agent_cfg.name).exists()
    ...
    return AgentResponse(
        ...
        soul=soul,
        category="public" if is_public else "custom",
    )
...
async def delete_agent(name: str) -> None:
    ...
    try:
        ...
        # 清理 memory 中提及该智能体名称的事实和摘要
        try:
            storage = get_memory_storage()
            memory_data = storage.load(agent_name=None, user_id=user_id)
            if memory_data:
                changed = False
                # 删除 facts 中包含智能体名称的条目
                facts = memory_data.get("facts", [])
                filtered = [f for f in facts if name.lower() not in f.get("content", "").lower()]
                if len(filtered) != len(facts):
                    memory_data["facts"] = filtered
                    changed = True

                # 清理 user/history 摘要中对该智能体的引用
                for section in ("user", "history"):
                    section_data = memory_data.get(section, {})
                    for key, val in section_data.items():
                        if isinstance(val, dict) and "summary" in val and val["summary"]:
                            # 只清除明确提及该智能体的句子，保留上下文
                            import re
                            cleaned = re.sub(
                            ...
                            if cleaned != val["summary"]:
                                val["summary"] = cleaned
                                changed = True

                if changed:
                    storage.save(memory_data, agent_name=None, user_id=user_id)
                    logger.info(f"Cleaned memory facts referencing agent '{name}'")
        except Exception as mem_err:
            logger.warning(f"Failed to clean memory for agent '{name}': {mem_err}")
    except Exception as e:
