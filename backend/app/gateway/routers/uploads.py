...
def _make_file_sandbox_writable(file_path: os.PathLike[str] | str) -> None:
    """Ensure uploaded files are readable and writable inside sandbox containers.

    In AIO sandbox mode, the gateway writes the authoritative host-side file
    first, then the sandbox runtime (which may run as a different UID) reads
    and rewrites it. The default file creation mode ``0o600`` is owner-only;
    sandbox processes cannot read such files. This helper grants world-read
    and world-write access (``0o666`` equivalent, minus any sticky bits).
    """
    ...
    # Grant world-read + world-write (sandbox UID may differ from gateway UID)
    current = stat.S_IMODE(file_stat.st_mode)
    sandbox_mode = current | stat.S_IRUSR | stat.S_IWUSR | stat.S_IRGRP | stat.S_IWGRP | stat.S_IROTH | stat.S_IWOTH
    chmod_kwargs = {"follow_symlinks": False} if os.chmod in os.supports_follow_symlinks else {}
    os.chmod(file_path, sandbox_mode, **chmod_kwargs)
