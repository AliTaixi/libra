...
DEFAULT_LANGGRAPH_URL = "http://localhost:8002/api"
DEFAULT_GATEWAY_URL = "http://localhost:8002"
...
def _resolve_attachments(thread_id: str, artifacts: list[str]) -> list[ResolvedAttachment]:
    ...
    Only paths under ``/mnt/user-data/`` are accepted; any other
    virtual path is rejected with a warning.
    ...
    for virtual_path in artifacts:
        ...
        if not virtual_path.startswith(_USER_DATA_VIRTUAL_PREFIX):
            ...
            created.append(
                {
                    "filename": safe_name,
                    "size": len(data),
                    "path": f"/mnt/user-data/{safe_name}",
